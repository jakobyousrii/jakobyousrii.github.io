...
// const PokeLocation = async () => {

//     const awaitLocation = await axios.get("https://pokeapi.co/api/v2/pokemon-habitat");

//     for (let data of awaitLocation.data.results) {
...
//         h4.classList.add("h4Location");
//         for (let loc in locationsPic) {
//             if (loc === data.name) {
//                 content.style.backgroundColor = `${locationsPic[loc]}`;

//             }
//         }
//         content.classList.add("locationArticle");
//         content.append(h4, button);
//         locationSection.append(content);

//         try {
//             //pokemon species
//             const pokeHabi = await axios.get(data.url);
//             const allPoke = document.createElement("article");
//             allPoke.classList.add("pokeLocations");

//             for (let poke of pokeHabi.data.pokemon_species) {
//                 const pokeSpecies = await axios.get(poke.url);

//                 button.addEventListener("click", async (e) => {

//                     if (e.target.nodeName = "I") {
//                         allPoke.classList.toggle("display");

//                         if (allPoke.classList.contains("display")) {

//                             button.innerHTML = `<i class="far fa-times-circle margin-far"></i>`;

//                             for (let pika of pokeSpecies.data.varieties) {
//                                 const imgAndName = document.createElement("div");
//                                 imgAndName.classList.add("pokeNameLocation");
//                                 const pikaUrl = pika.pokemon.url;
//                                 const imgReq = await axios.get(pikaUrl.slice(0, -1));
//                                 if (imgReq.data.sprites.front_default) {
//                                     const img = document.createElement("img");
//                                     const name = document.createElement("h4");
//                                     name.textContent = imgReq.data.species.name;
//                                     img.src = imgReq.data.sprites.front_default;
//                                     img.classList.add("imgClassTypes");
//                                     imgAndName.append(img, name);
//                                     allPoke.append(imgAndName);

//                                 }
//                             }
//                             content.append(allPoke);
//                         }

//                         else {
//                             button.innerHTML = `<i class="fas fa-chevron-circle-down margin-far"></i>`;
//                             const pokeRm = document.querySelectorAll(".pokeNameLocation");
//                             for (let pk of pokeRm) {
//                                 pk.remove();
//                             }
//                         }
//                     }
